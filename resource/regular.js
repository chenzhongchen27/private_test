...
/***/ (function(module, exports, __webpack_require__) {
	...
	_.createProto = function(fn, o){
	  //创建一个对象Foo
	  //这个对象的constructor是第一个参数fn 
	  //原型是第二个参数 o
	  //同时将第一个参数fn的原型也设为这个实例
	  //是在：fn的原型设为o，并将其返回从而可以编辑
...
/***/ (function(module, exports, __webpack_require__) {
	...
	*/
	...
	Regular.implement({
	  ...
	  // 2. list里的循环
	  // 在watcher.js的this.$expression中用到。
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function extend(o){
	  ...
	  function implement(o){
	    ...
	    var len = mlen;
	    for(;len--;){
	      var prop = merged[len]; //为 data 或 computed
	      //如果原型有，对象本身也有，那就把对象的赋值到原型上。删除对象本身的。
	      //对Regular来说，proto为Regular的原型 
	      if(proto[prop] && o.hasOwnProperty(prop) && proto.hasOwnProperty(prop)){
	        //当原型有，函数本身也有时，以函数本身的为准。
	...
	var API = {
	  $on: function(event, fn, desc) {
	    if(typeof event === "object" && event){
	      ...
	      //calls为this._handles[event],即某一个事件的所有函数。或者一个空数组
	        calls = handles[event] || (handles[event] = []);
	      var realFn;
	      if(desc.once){
	        //当一次时，放realFn函数。这个是运行一次，然后就解绑 并将该过程放到fn的real上面
	        realFn = function(){
	      ...
	      //当desc有once时，calls里放realFn(带解绑的)。否则放fn。
	  ...
	  $emit: function(event){
	    ...
	    if(!event) return;
	    //第一个是 event 的名字，所以只会从 1 开始选参数
	    var args = _.slice(arguments, 1);
	    var type = event;

	    if(!handles) return context;
	    //type.slice(1)是将第一个字母 _ 去掉，如果 存在，则用 calls[j].apply 全部遍历一遍
	  ...
	  $once: function(event, fn){
	    var args = _.slice(arguments);
	    //在 $on 中会进行判断，如果第三个参数是 desc.once ，则只执行一次。
	    args.push({once: true})
